...
/*********************************************************************************
*
*			Module Name:		calc_comp_ad.c
*			Revision Number:	2.4
*			Date last revised:	6 December 2011
*			Programmer:			Steven Gillispie
*			Date Originated:	9 August 2011
*
*			Module Overview:	Conversion of MK contributed calc_comp_ad.f to C
*
*			References:			
*
**********************************************************************************
...
static double			e, rho, A[kElems];

static int				i, ie, iz, n, iztab, ientry, nentry[kElems], neg1;
static char				compound[30], out_file[30];
static Boolean			m_mask[kElems];

static double			sf[kElems][3001], percent;
static double			x[kElems][3001], w_frac[kElems];
static double			x1, energy, lambda, lambda2, theta_step = 1.0e-4;
static double			e2wave = 1.239852e-7, t = 0.6652448;
static double			sf1, sf2, form, pi = 3.1415926536;
static double			sig_cum[3001], theta, cos2_theta;
...
*			double*			y			- 
*
*		Function return: double -- Interpolated value.
*
*********************************************************************************/
...

{
	...
	if (z < x[2]) {
		...
		result = ((z - x[1])*y[2] + (x[2]-z)*y[1])/(x[2] - x[1]);
	}
	else {
		i = 2;
		while (x[i] < z) {
...

{
	...
	for (itheta=1; itheta<=ntheta; itheta++) {
		...
		for (iz=1; iz<kElems; iz++) {
			...
			sf1 = w_frac[iz] * interp(x1, x[iz], sf[iz]);
			sig[itheta] += sf1;
		}
		...
		/* EF = ENERGY FRACTION (E/511) ; CT = COS(THETA) */
		
		term = ef * (1.0 - ct);
		sig[itheta] *= (1 + (ct*ct) + (term*term/(1.0 + term)));
		sig[itheta] /= ((1.0 + term)*(1.0 + term));
		sig_cum[ie] += sig[itheta];
	}
	
	for (itheta=2; itheta<=ntheta; itheta++) {
		sig[itheta] += sig[itheta-1];
	}
	
	for (itheta=1; itheta<=ntheta; itheta++) {
		sig[itheta] /= sig_cum[ie];
	}
	
	itheta = 1;
	for (ipercent=1; ipercent<=100; ipercent++) {
		percent = ipercent/100.0;
		while (sig[itheta] < percent) {
			itheta++;
		}
...

{
	...
	infileSF8 = fopen( "SF.dat", "r" );
	if ( ! infileSF8 ) {
		printf( "\tCould not open SF.dat\n" );
		exit( 8 );
	}
	...
	for (iz=1; iz<kElems; iz++ ) {
		...
		while (iztab == iz) {
			fscanf( infileSF8, "%d", &iztab );
			if (iztab < 0) {
				break;
			}
			fscanf( infileSF8, "%lf %lf", &x1, &sf1 );
			ientry++;
			if (ientry > 1000) {
